...
extension UIView {
  /// Use this method on a new instance of UIView or its subclass to prepare for AutoLayout (sets `translatesAutoresizingMaskIntoConstraints` to `false`)
  ...
  public func verticalAlign(
    edge: LayoutYAxisEdge,
    toEdge: LayoutYAxisEdge,
    of view: AnchorsProviding,
    with offset: CGFloat = 0.0,
    priority: UILayoutPriority = .required
  ) -> NSLayoutConstraint {
    let constraint = anchor(for: edge).constraint(equalTo: view.anchor(for: edge), constant: offset)
    constraint.priority = priority
    constraint.isActive = true
    return constraint
  }
